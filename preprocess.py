...
def get_src_landmarks_t(x0, x1, y0, y1, pnts):
    """
...
def process_image(input_img, info, detector, save_interval, save_path):
    ...
    if frame % save_interval == 0:
        faces, pnts = detector.detect_face(
            input_img, threshold=detec_threshold, use_auto_downscaling=False)
        for idx, (x0, y1, x1, y0, conf_score) in enumerate(faces):
            det_face_im = input_img[int(x0):int(x1), int(y0):int(y1), :]

            # get src/tar landmarks
            src_landmarks = get_src_landmarks_t(x0, x1, y0, y1, pnts)
            ...
            Path(os.path.join(f"{save_path}", "rgb")).mkdir(
                parents=True, exist_ok=True)
            fname = os.path.join(f"{save_path}", "rgb",
                                 f"frame{frame}face{str(idx)}.jpg")
            ...
            h, w = bm.shape[:2]
            bm[
                int(src_landmarks[0][0] - h / 15):int(src_landmarks[0][0] + h / 15),
                int(src_landmarks[0][1] - w / 8):int(src_landmarks[0][1] + w / 8),
                :] = 255
            bm[
                int(src_landmarks[1][0] - h / 15):int(src_landmarks[1][0] + h / 15),
                int(src_landmarks[1][1] - w / 8):int(src_landmarks[1][1] + w / 8),
                :] = 255
            bm = landmarks_match_mtcnn(bm, src_landmarks, tar_landmarks)
            Path(os.path.join(f"{save_path}", "binary_mask")).mkdir(
                parents=True, exist_ok=True)
            fname = os.path.join(
                f"{save_path}", "binary_mask", f"frame{frame}face{str(idx)}.jpg")
...
def preprocess_video(fn_input_video, fd, save_interval, save_path):
    ...
    clip = clip1.fl_image(lambda img: process_image(
        img, info, fd, save_interval, save_path))
