...
class Converter:
    ...
    def process_video(self, frame, config: ConverterConfig):
        ...
        logger.debug("Detected %i face(s) in a frame and point count %i" %
                     (len(faces), len(pnts)))
        ...
        for i, (x0, y1, x1, y0, conf_score) in enumerate(faces):
            ...
            if config.use_smoothed_bbox:
                ...
                if self._frames != 0 and conf_score >= best_conf_score:
                    logger.debug(
                        "cond: frames != 0 and conf_score >= best_conf_score")
    ...
    def _transform(self, frame, image_shape, face,
        ...
        try:
            ...
            aligned_det_face_im = landmarks_match_mtcnn(
                detected_face_img, src_landmarks, tar_landmarks)
            ...
            # reverse alignment
            rev_aligned_det_face_im_rgb = landmarks_match_mtcnn(
                r_rgb, tar_landmarks, src_landmarks)
            rev_aligned_mask = landmarks_match_mtcnn(
                r_alpha, tar_landmarks, src_landmarks)
            ...
            # TODO: Maybe needed to reverse masking for aligned_im_a and aligned_im_b
            #  (which does no swap).
            aligned_im_a: np.ndarray = (
                1 - rev_aligned_mask / 255) * seg_im_rgb
            aligned_im_b: np.ndarray = (
                rev_aligned_mask / 255) * detected_face_img
        ...
        comb_img[int(x0):int(x1), frame.shape[1] + int(y0)
                     :frame.shape[1] + int(y1), :] = result

        # Enhance output
        if config.enhance != 0:
            comb_img = -1 * config.enhance * \
                get_init_comb_img(frame) + (1 + config.enhance) * comb_img
